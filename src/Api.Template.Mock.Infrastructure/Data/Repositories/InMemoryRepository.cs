...
namespace Api.Common.Mock.Infrastructure.Data.Repositories
{
    public class InMemoryRepository<TEntity> : IRepository<TEntity> where TEntity : IDomainEntity
    {
        ...
        {
            foreach (var instance in instances) Insert(instance);
        }
        ...
        {
            foreach (var instance in instances) Update(instance);
        }

        public TEntity Find(Expression<Func<TEntity, bool>> expression)
        {
            return Repository.Any() ? Repository.AsQueryable().First(expression) : default;
        }
